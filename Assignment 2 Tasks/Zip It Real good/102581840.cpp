...
// Function that calucales sum1 total
int sum1(int sumOne[]) {
    int sumOneTotal = 0;
    cout << "" << endl;
...
// Function that calculates sum2 total
int sum2(int sumTwo[]) {
    int sumTwoTotal = 0;
...
int main() {
    ...
    // Loop that enters in credit card numbers into array
    for (int i = 0; i < 20; i++) {
        cin >> creditCard[i];
    ...
    // Loop that prints out credit card numbers to screen
    for (int j = 0; j < 20; j++) {
        if (creditCard[j] == -1 ) {
            break;
        }
